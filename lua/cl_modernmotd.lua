...
function MODERN.OpenMOTD( settings )
	...
	if !ModernMainWindow then
		...
		local opentab = settings.opentab or MODERN.Settings.OpenTab
		if MODERN.TabManager[opentab] then 
		MODERN.ShowTabPanel(MODERN.TabManager[opentab].panel,
		MODERN.TabManager[opentab].btn,MODERN.TabManager[opentab].url)
...
function MODERN.PaintMainWindow()
	...
	if MODERN.Settings.FKeyBind and MODERN.Settings.FKeyBind != "" 
